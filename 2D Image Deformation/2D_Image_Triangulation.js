...
export async function imageTriangulate()
{
    // initial points
    // 1. set square points
    // 2. set random points
...
function clacDelunay(imageContext, initPoints, depth)
{
    // 초기의 배열 복사
...
function getAverageColor(ctx, p1, p2, p3) {
    const [x1, y1] = p1;
    const [x2, y2] = p2;
    const [x3, y3] = p3;
...
function getLowErrorPoint(
    ...
    for (let y = minY; y <= maxY; y++) {
        for (let x = minX; x <= maxX; x++) {
            if (isInsideTriangle({x1, y1, x2, y2, x3, y3, px: x, py: y })){
                const index = (y * width + x) * 4; // RGBA 인덱스 계산
